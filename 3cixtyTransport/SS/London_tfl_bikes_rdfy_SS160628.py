...
def getBikeData(row):
    ...
    #timeModified = datetime.datetime.strptime(row[5],"%Y-%m-%dT%H:%M:%S.%f%z")
    nFilledDocks = row[6]
    nEmptyDocks = row[7]
    nTotalDocks = row[8]
    lat=row[9]
...
def createBikeGraph(arg, g):
    ...
    bikeAddressStreet = Literal(bikeAddress.split(',', 1)[0])
    bikeAddressLocality = Literal(bikeAddress.split(',', 1)[1].replace(' ', '',1))
...
def main():
    ...
    outFile = pathf + "tfl_bikes" + strftime("%Y%m%d") + "_toVal.ttl"
