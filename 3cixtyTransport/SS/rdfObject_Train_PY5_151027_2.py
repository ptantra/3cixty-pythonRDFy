...
def createTrainGraph(arg,g): #+TICKED
  ...
  # trans = Namespace("http://vocab.linkeddata.es/datosabiertos/def/urbanismo-infraestructuras/Transporte#")
    ...
    singleStation2 = createTrainStation(arg[4].replace(" ", ""))
    singleAddress = createAddress(arg[0], arg[5]) ##check arg[5]#singleStation = self.createTrainStation(stationid)
    singleGeometry = createGeometry(arg[0], arg[6].replace(" ", "")) #station names without spaces
    transitRouteValue = URIRef("http://data.linkedevents.org/transit/London/railwayRoute/%s") % arg[4].replace(" ", "")
    ...
    g.add((singleStation, geo.location, singleGeometry)) #g.add((singleStation, schema.location, singleAddress))
