...
class RDFclass_bikes:
    ...
    def json2csv(self,content): #or is it jsoncontent?
        parser = init_parser()
    ...
    def createRDF(self,row):
        ...
        unknown = Namespace(nspaces.get('unknown'))
    ...
    def main():
        ...
        inputFile = pathf + "bikes.csv"
        outFile = pathf + "bikes.ttl"

        csvBike = readCsv(inputFile)
        ...
        for row in csvBike:
            lst = getBikeData(row)
            createRDF(lst, bike_g, flag)
            flag = 0
        createRDF(lst, bike_g, flag).serialize(outFile, format='turtle')
