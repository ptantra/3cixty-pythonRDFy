...
class RDFclass:
    ...
    def createTrainBranch(self,stopId):
        singleStop = URIRef("http://data.linkedevents.org/transit/London/branch/" + stopId)
        return singleBranch
    ...
    def creatRDF(self,row):
        ...
        stopLat,stopLong=convertProj(row[5],row[6])
        #stopLat,stopLong=convertProj(row[4],row[5])

        noAddress=""
        stopGeometry = "POINT ("+str(stopLat) +" "+str(stopLong)+")"
        stopRoute = URIRef('http://data.linkedevents.org/transit/London/trainRoute/')
        stopGUID = uid
        ...
        singleBranch =self.createTrainBranch(stopid)
        ...
        self.g.add((singleStop, transit.railRoute, stopRoute))
