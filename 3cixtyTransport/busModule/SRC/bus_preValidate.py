...
def writeLogP(status,row):
    ...
    if status==1:
        row.append('P1')
        writeCsv(busPathf + 'DATA/LOG/bus_errorLog_p1.csv',row)
        return 1
    elif status==2:
        row.append('P2')
        writeCsv(busPathf + 'DATA/LOG/bus_errorLog_p2.csv',row)
        return 2

def deleteFile(filename):
    filename = busPathf + 'DATA/bus_validated.csv'
    try:
        os.remove(filename)
    except OSError:
        pass
...
def writeCleaned(row):
    ...
    writeCsv(busPathf + 'DATA/bus_validated.csv', row)
    return 0
...
def convertProj(rowLon,rowLat):##BUS
    for index in range(0, len(rowLon, rowLat)):
        Bng = pyproj.Proj(init='epsg:27700')
        Wgs84 = pyproj.Proj(init='epsg:4326')
        #print (lat+'-'+lon)
        wgsLon,wgsLat = pyproj.transform(Bng,Wgs84,rowLon, rowLat)
    return wgsLon,wgsLat
...
if __name__ == "__main__":
    ...
    busPathf = "./"
    inFileB = busPathf + "DATA/bus-stops.csv"
 ...
 #delete the last line if it contains the ASCII code
    if busData[0] == '\x1a' and busData[1:].isnull().all():
        busData = busData[:-1]#look through csv and clean the last row if ASCII or null
    ...
    if os.path.exists(busPathf + 'DATA/bus_validated.csv'):
        os.remove(busPathf + 'DATA/bus_validated.csv')
