...
def writeLogP(status,row):
    ...
    if status==1:
        row.append('P1')
        writeCsv(busPathf + 'busModule/IN/validation/bus_errorLog_p1.csv',row)
        return 1
    elif status==2:
        row.append('P2')
        writeCsv(busPathf + 'busModule/IN/validation/bus_errorLog_p2.csv',row)
        return 2
...
def writeCleaned(row):
    ...
    writeCsv(busPathf + 'busModule/IN/validation/bus_validated.csv', row)
    return 0
...
if __name__ == "__main__":
    ...
    busPathf = "./"
    inFileB = busPathf + "busModule/IN/bus-stops.csv"
 ...
 #delete the last line if it contains the ASCII code
    ...
    if os.path.exists(busPathf + 'busModule/IN/validation/bus_validated.csv'):
        os.remove(busPathf + 'busModule/IN/validation/bus_validated.csv')
