...
def getBusData(row):
    ...
    if not os.path.exists('OUT/validation/'):
        os.makedirs(strftime('OUT/validation/'))
...
def createBusGraph(arg,g):
    ...
    singleAddress = createAddress(arg[1])
    singleGeometry = createGeometry(arg[1])
    ...
    g.add((singleGeometry, geo.lon, Literal(arg[1], datatype=xsd.placeholder)))
...
def main():
    ...
    outFileB= busPathf + 'OUT/validation/bus_dirty.ttl'
