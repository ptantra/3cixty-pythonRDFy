...
def createStation(stationName):  # createTubeGraph
    stationName = URIRef(
        'http://data.linkedevents.org/transit/london/subwayStop/' + Literal(stationName).replace(" ", "").replace("(",
                                                                                                                  "").replace(
            ")", "").replace("-", "_").replace('&', "And").replace("&amp;", "And"))
    # stationName = URIRef('http://data.linkedevents.org/transit/london/subwayStop/' + Literal(stationName).replace([" (-"], "").replace('&', "And").replace("&amp;", "And"))
    return stationName
...
def main():
    ...
    inFileTube = 'IN/validation/dlr_validated.csv'
    outFileTube = "OUT/dlr_dirty" + ".ttl"
