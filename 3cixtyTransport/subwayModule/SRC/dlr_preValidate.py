...
def writeLogP(status,row):
    ...
    if status==1:
        ...
        writeCsv(pathf + 'DATA/dlr_errorLog_p1.csv',row)
        return 1
    elif status==2:
        row.append('Error: Non numeric and/or digit value')
        writeCsv(pathf + 'DATA/dlr_errorLog_p2.csv',row)
        return 2
    elif status == 3:
        row.append('Error: Invalid URL')
        writeCsv(pathf + 'DATA/dlr_errorLog_p3.csv', row)
        return 3
    elif status == 4:
        row.append('Error: Equal values Invalid URL')
        writeCsv(pathf + 'DATA/dlr_errorLog_p4.csv', row)
        return 4
    elif status == 5:
...
def writeCleaned(row):
    ...
    writeCsv(pathf + 'DATA/dlr_validated.csv', row)
    return 0
...
if __name__ == "__main__":
    ...
    inFileB = pathf + "DATA/dlr_ALL.csv"
    ...
    if os.path.exists(pathf + 'DATA/dlr_validated.csv'):
        os.remove(pathf + 'DATA/dlr_validated.csv')
