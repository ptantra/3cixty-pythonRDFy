...
def writeLogP(status,row):
    if status==1:
        ...
        writeCsv('IN/' + strftime("%Y%m%d") + '/validation/bike_errorLog_p3.csv', row)
        return 3
    elif status == 4:
        row.append('Error: Equal values Invalid URL')
        writeCsv('IN/' + strftime("%Y%m%d") + '/validation/bike_errorLog_p4.csv', row)
        return 4

def deleteFile(filename):
    filename = 'IN/validation/bike_validated.csv'
    try:
        os.remove(filename)
    except OSError:
        pass
...
def writeCleaned(row):
    ...
    writeCsv('IN/'+ strftime("%Y%m%d") +'/validation/bike_validated.csv', row)
    return 0
...
if __name__ == "__main__":
    ...
    inFileB = "IN/tfl_bikes20160728.csv"
    ...
    if os.path.exists('IN/' + strftime("%Y%m%d") + '/validation/bike_validated.csv'):
        os.remove('IN/' + strftime("%Y%m%d") + '/validation/bike_validated.csv')
