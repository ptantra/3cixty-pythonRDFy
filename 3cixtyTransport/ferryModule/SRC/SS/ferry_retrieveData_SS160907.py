...
for lineNo in xrange(len(lineId)):
    ...
    outXmlLines = "IN/"+ strftime("%Y%m%d") +"/ferryLine_"+lineId[lineNo] +".xml"
    ...
    def apiJsonCsv(inUrlLines):
        ...
        fMain = csv.writer(open("IN/"+ strftime("%Y%m%d") +"/ferryLine_" + lineId[lineNo] + ".csv", "wb+"))
    ...
    ferry = DataFrame(pd.read_csv("IN/"+ strftime("%Y%m%d") +"/ferryLine_" + lineId[lineNo] + ".csv"))


    #print join
    ferry.to_csv("IN/"+ strftime("%Y%m%d") +"/ferryLine_" + lineId[lineNo] + "_output.csv", index=False)

rb1 = DataFrame(
    pd.read_csv("IN/" + strftime("%Y%m%d") + "/ferryLine_" + lineId[0] + "_output.csv"))
rb2 = DataFrame(
    pd.read_csv("IN/" + strftime("%Y%m%d") + "/ferryLine_" + lineId[1] + "_output.csv"))
rb4 = DataFrame(
    pd.read_csv("IN/" + strftime("%Y%m%d") + "/ferryLine_" + lineId[2] + "_output.csv"))
rb5 = DataFrame(
    pd.read_csv("IN/" + strftime("%Y%m%d") + "/ferryLine_" + lineId[3] + "_output.csv"))
rb6 = DataFrame(
    pd.read_csv("IN/" + strftime("%Y%m%d") + "/ferryLine_" + lineId[4] + "_output.csv"))
westminsterPassenger = DataFrame(
    pd.read_csv("IN/" + strftime("%Y%m%d") + "/ferryLine_" + lineId[5] + "_output.csv"))
woolwichFerry= DataFrame(
    pd.read_csv("IN/" + strftime("%Y%m%d") + "/ferryLine_" + lineId[6] + "_output.csv"))


lines = rb1.append([rb2, rb4, rb5,
                         rb6, westminsterPassenger, woolwichFerry], ignore_index=True)

lines.to_csv("IN/_"+ strftime("%Y%m%d") +"/ferry.csv", index=False)
