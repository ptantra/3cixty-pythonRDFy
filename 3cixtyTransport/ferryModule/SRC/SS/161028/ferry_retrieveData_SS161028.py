...
for lineNo in xrange(len(lineId)):
    ...
    outXmlLines = "IN/ferryLine_"+lineId[lineNo] +".xml"
    ...
    def apiJsonCsv(inUrlLines):
        ...
        fMain = csv.writer(open("IN/ferryLine_" + lineId[lineNo] + ".csv", "wb+"))
    ...
    ferry = DataFrame(pd.read_csv("IN/ferryLine_" + lineId[lineNo] + ".csv"))
    #print join
    ferry.to_csv("IN/ferryLine_" + lineId[lineNo] + "_output.csv", index=False)

rb1 = DataFrame(
    pd.read_csv("IN/ferryLine_" + lineId[0] + "_output.csv"))
rb2 = DataFrame(
    pd.read_csv("IN/ferryLine_" + lineId[1] + "_output.csv"))
rb4 = DataFrame(
    pd.read_csv("IN/ferryLine_" + lineId[2] + "_output.csv"))
rb5 = DataFrame(
    pd.read_csv("IN/ferryLine_" + lineId[3] + "_output.csv"))
rb6 = DataFrame(
    pd.read_csv("IN/ferryLine_" + lineId[4] + "_output.csv"))
...
lines.to_csv("./IN/ferry.csv", index=False)
