...
def deleteFile(filename):
    filename = 'DATA/airbnb_validatedSmall.csv'
    try:
        os.remove(filename)
    except OSError:
        pass
...
def writeCleaned(row):
    ...
    writeCsv('DATA/airbnbLondon_validatedSmall.csv', row)
    return 0
...
if __name__ == "__main__":
    ...
    pathf = "./"
    inFileB = pathf + "DATA/airbnbLondonSmall.csv"
 ...
 #delete the last line if it contains the ASCII code
    ...
    if os.path.exists(pathf + 'DATA/airbnblondon_ValidatedSmall.csv'):
        os.remove(pathf + 'DATA/airbnblondon_ValidatedSmall.csv')
