...
if not os.path.exists('DATA/LOG/'):
    os.makedirs(strftime('DATA/LOG/'))

if not os.path.exists('DATA/FEEDS/'):
    os.makedirs(strftime('DATA/FEEDS/'))
...
def writeCleaned(row):
    ...
    writeCsv('DATA/LOG/wunderground_validated.csv', row)
    return 0

if __name__ == "__main__":
    ##create non duplicate file
    #inFile = open('IN/stations_full.csv', 'r')
    inFile = open('DATA/stations_fullNoDuplicate.csv','r')
    outFile = open("DATA/wunderground_validated.csv", 'w')
    listLines = []
    ...
    inFileB = "DATA/stations_fullNoDuplicate.csv"
