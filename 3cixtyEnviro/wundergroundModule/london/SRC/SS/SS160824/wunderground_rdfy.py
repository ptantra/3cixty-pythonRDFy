...
import csv, uuid, pyproj, random, string, time, sys, fileinput
...
pathf = "./"
inFile = pathf + "IN/stations_fullNoDuplicate.csv"
...
def createWeatherDeviceId(getUid):
    weatherId = URIRef("http://data.linkedevents.org/environment/London/wunderground/" + Literal(getUid))
    return weatherId
...
def createWeatherGraph(arg, g):
    ...
    g.add((geom, locn.geometry, Literal(arg[5], datatype=xsd.wktLiteral)))
    ...
    g.add((forecast, rdfs.comment, Literal('Returns a summary of the weather for the next 3 days. This includes high and low temperatures, a string text forecast and the conditions.', lang='en')))
...
def main():
    #pathf = "Z:/3cixty/3cixty_160816/3cixtyEnviro/QEOPIntel/"
    inFile = pathf + "IN/stations_full.csv"
    outFile = pathf + "IN/stations_dirty.ttl"
