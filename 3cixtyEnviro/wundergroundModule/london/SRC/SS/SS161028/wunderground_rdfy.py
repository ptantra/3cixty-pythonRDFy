...
def createWeatherDeviceId(getUid):
    weatherId = URIRef("http://data.linkedevents.org/wunderground/observation/" + Literal(getUid))
    return weatherId

def createArea(weatherDeviceCode):
    area = URIRef('http://data.linkedevents.org/wunderground/deviceId/' + Literal(weatherDeviceCode))
    return area
...
def main():
    ...
    inFile = 'IN/validation/wunderground_validated.csv'
    outFile = 'IN/wunderground_dirty.ttl'
