...
def getWeatherData(row):
    ...
    timestamp = Literal(datetime.fromtimestamp(float(row[1])).strftime("%Y-%m-%dT%H:%M:%S"))
    condition = Literal(str(row[2]))
...
def createWeatherGraph(arg, g):
    ...
    g.add((forecast, dct.description, Literal('weather condition', lang='en')))
...
def main():
    ...
    inFile = "IN/" + strftime("%Y%m%d") + "/wunderground_realTime.csv"
    outFile = "OUT/"+ strftime("%Y%m%d") +"/wunderground_realTime.ttl"
