...
def createArea(airQualCode):
    area = URIRef('http://data.linkedevents.org/environment/London/laqn/stationCode/' + Literal(airQualCode))
    return area

def createGeom(airQualCode):
    geom = createArea(airQualCode) + '/geometry'
    return geom

def createProvisionalOrRatified(airQualRatified):
    provisional = Literal('Provisional')
    ratified = Literal('Ratified')
    if airQualRatified == str('P'):
        return provisional
    elif airQualRatified == str('R'):
        return ratified
...
def createAirQualGraph(arg, g):
    ...
    eg = Namespace('http://example.org/example/')

    area = createArea(arg[0])
    geom = createGeom(arg[0])
    ds_pm10 = createAirQualityId(arg[1], arg[3])
    provOrRatified=createProvisionalOrRatified(arg[7])

    g.add([eg.dsd_le, rdf.type, qb.DataStructureDefinition])
    g.add([eg.dsd_le, qb.component, qb.measure])
    g.add([eg.dsd_le, qb.measure, eg.lifeExpectancy])


    #g.add((area, rdf.type, dul.Place))
    #g.add((area, rdf.type, schema.Place))
    #g.add((area, rdf.type, qb.ComponentProperty))
    #g.add((area, rdfs.label, Literal(arg[2])))
    #g.add((area, rdfs.range, Literal('interval')))
    ...
    #g.add((area, qb.attribute, provOrRatified))
...
def main():
    ...
    inFile = pathf + "londonPM25merge.csv"
    outFile = pathf + "londonPM25merge.ttl"
